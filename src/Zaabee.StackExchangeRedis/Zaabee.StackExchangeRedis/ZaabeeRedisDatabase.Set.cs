...
{
    public bool SetAdd<T>(string key, T? value) => _db.SetAdd(key, (RedisValue)_serializer.ToBytes(value));
    ...
    public bool SetContains<T>(string key, T? value) => _db.SetContains(key, _serializer.ToBytes(value));

    public long SetLength<T>(string key) => _db.SetLength(key);

    public IList<T> SetMembers<T>(string key) => _db.SetMembers(key)
        .Select(value => value.HasValue ? _serializer.FromBytes<T>(value) : default).ToList();
    ...
    public bool SetRemove<T>(string key, T? value) => _db.SetRemove(key, _serializer.ToBytes(value));
