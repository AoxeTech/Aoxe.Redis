...
public partial class ZaabeeRedisDatabase
{
    ...
    public long SortedSetAdd<T>(string key, IDictionary<T, double> values) => _db.SortedSetAdd(key,
        values.Select(value => new SortedSetEntry(_serializer.ToBytes(value.Key), value.Value)).ToArray());
    ...
    public bool SortedSetRemove<T>(string key, T? member) => _db.SortedSetRemove(key, _serializer.ToBytes(member));
    ...
    {
        var values = _db.SortedSetScan(key, _serializer.ToBytes(pattern), pageSize, cursor, pageOffset);
        return values.ToDictionary(k => _serializer.FromBytes<T>(k.Element), v => v.Score);
    }
