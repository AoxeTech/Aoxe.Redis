...
namespace Zaabee.StackExchangeRedis
{
    public partial class ZaabeeRedisDatabase
    {
        ...
        public bool SetContains<T>(string key, T value) => _db.SetContains(key, _serializer.Serialize(value));

        public long SetLength<T>(string key) => _db.SetLength(key);

        public IList<T> SetMembers<T>(string key) => _db.SetMembers(key)
            .Select(value => value.HasValue ? _serializer.Deserialize<T>(value) : default).ToList();

        public bool SetMove<T>(string source, string destination, T value) =>
            _db.SetMove(source, destination, _serializer.Serialize(value));
        ...
        public bool SetRemove<T>(string key, T value) => _db.SetRemove(key, _serializer.Serialize(value));
