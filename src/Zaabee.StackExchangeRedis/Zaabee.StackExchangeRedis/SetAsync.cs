...
namespace Zaabee.StackExchangeRedis
{
    public partial class ZaabeeRedisDatabase
    {
        ...
        public async Task<IList<T>> SetCombineUnionAsync<T>(string firstKey, string secondKey)
        {
            ...
            return values.Select(value => value.HasValue ? _serializer.Deserialize<T>(value) : default).ToList();
        }
        ...
        public async Task<bool> SetContainsAsync<T>(string key, T value) =>
            await _db.SetContainsAsync(key, _serializer.Serialize(value));
        ...
        public async Task<IList<T>> SetMembersAsync<T>(string key)
        {
            ...
            return results.Select(value => value.HasValue ? _serializer.Deserialize<T>(value) : default)
                .ToList();
        }

        public async Task<bool> SetMoveAsync<T>(string source, string destination, T value) =>
            await _db.SetMoveAsync(source, destination, _serializer.Serialize(value));
        ...
        public async Task<bool> SetRemoveAsync<T>(string key, T value) =>
            await _db.SetRemoveAsync(key, (RedisValue) _serializer.Serialize(value));

        public async Task<long> SetRemoveRangeAsync<T>(string key, IEnumerable<T> values) =>
            await _db.SetRemoveAsync(key, values.Select(value => (RedisValue) _serializer.Serialize(value)).ToArray());
