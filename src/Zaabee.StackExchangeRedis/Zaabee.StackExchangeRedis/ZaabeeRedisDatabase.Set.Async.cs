...
namespace Zaabee.StackExchangeRedis
{
    public partial class ZaabeeRedisDatabase
    {
        public async Task<bool> SetAddAsync<T>(string key, T value) =>
            await _db.SetAddAsync(key, _serializer.SerializeToBytes(value));
        ...
        public async Task<bool> SetContainsAsync<T>(string key, T value) =>
            await _db.SetContainsAsync(key, _serializer.SerializeToBytes(value));
        ...
        public async Task<IList<T>> SetMembersAsync<T>(string key)
        {
            ...
            return results.Select(value => value.HasValue ? _serializer.DeserializeFromBytes<T>(value) : default)
                .ToList();
        }

        public async Task<bool> SetMoveAsync<T>(string source, string destination, T value) =>
            await _db.SetMoveAsync(source, destination, _serializer.SerializeToBytes(value));
        ...
        public async Task<bool> SetRemoveAsync<T>(string key, T value) =>
            await _db.SetRemoveAsync(key, (RedisValue) _serializer.SerializeToBytes(value));

        public async Task<long> SetRemoveRangeAsync<T>(string key, IEnumerable<T> values) =>
            await _db.SetRemoveAsync(key, values.Select(value => (RedisValue) _serializer.SerializeToBytes(value)).ToArray());
            ...
            long cursor = 0, int pageOffset = 0)
        {
            var values = await Task.Factory.StartNew(() =>
                _db.SetScan(key, _serializer.SerializeToBytes(pattern), pageSize, cursor, pageOffset));
            return values.Select(value => value.HasValue ? _serializer.DeserializeFromBytes<T>(value) : default).ToList();
        }
    }
}